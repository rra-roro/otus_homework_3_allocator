...
struct mock_custom_allocator
{
      ...
      T* foo1(size_t n)
      {
            std::cout << "QQQQQ1\n";
            T* ret = Al.allocate(n);            
            EXPECT_THAT(this->Al.chunks->empty(), Eq(false));
            return ret;
      }
      T* foo2(size_t n)
      {
      ...
      void boo1(T* p, std::size_t n)
      {
            std::cout << "WWWWWWW1\n";
            Al.deallocate(p,n);
            EXPECT_THAT(this->Al.chunks->empty(), Eq(false));            
      }
      void boo2(T* p, std::size_t n)
      {
            std::cout << "WWWWWWW2\n";
            Al.deallocate(p, n);
            EXPECT_THAT(this->Al.chunks->empty(), Eq(false));
      }
      void boo3(T* p, std::size_t n)
      {
            std::cout << "WWWWWWW3\n";
            Al.deallocate(p, n);
            EXPECT_THAT(this->Al.chunks->empty(), Eq(false));
      }
      void boo4(T* p, std::size_t n)
      {
            std::cout << "WWWWWWW4\n";
            Al.deallocate(p, n);
            EXPECT_THAT(this->Al.chunks->empty(), Eq(false));
      ...
      void DelegateToReal()
      {
            ...
            else
            {
                  ...
                  EXPECT_THAT(this->bar, Eq(3));*/
                  using ::testing::InSequence;                        
                  {
                        InSequence dummy;

                        EXPECT_CALL(*this, allocate(1)).
                            .WillOnce(Invoke(this, &mock_custom_allocator<T, initial_reservation, next_reservation>::foo1));
                        EXPECT_CALL(*this, allocate(1))
                            .WillOnce(Invoke(this, &mock_custom_allocator<T, initial_reservation, next_reservation>::foo2));
                        EXPECT_CALL(*this, allocate(1))
                            .WillOnce(Invoke(this, &mock_custom_allocator<T, initial_reservation, next_reservation>::foo3));
                        EXPECT_CALL(*this, allocate(1))
                            .WillOnce(Invoke(this, &mock_custom_allocator<T, initial_reservation, next_reservation>::foo4));
                        EXPECT_CALL(*this, deallocate(_,_))
                            .WillOnce(Invoke(this, &mock_custom_allocator<T, initial_reservation, next_reservation>::boo1));
                        EXPECT_CALL(*this, deallocate(_,_))
                            .WillOnce(Invoke(this, &mock_custom_allocator<T, initial_reservation, next_reservation>::boo2));
                        EXPECT_CALL(*this, deallocate(_,_))
                            .WillOnce(Invoke(this, &mock_custom_allocator<T, initial_reservation, next_reservation>::boo3));
                        EXPECT_CALL(*this, deallocate(_,_))
                            .WillOnce(Invoke(this, &mock_custom_allocator<T, initial_reservation, next_reservation>::boo4));

                        //EXPECT_CALL(*this, allocate(1)).WillOnce(Invoke(&Al, &custom_allocator<T, initial_reservation, next_reservation>::allocate));
                        //EXPECT_CALL(*this, allocate(1)).WillOnce(Invoke(&Al, &custom_allocator<T, initial_reservation, next_reservation>::allocate));
                        //EXPECT_CALL(*this, allocate(1)).WillOnce(Invoke(&Al, &custom_allocator<T, initial_reservation, next_reservation>::allocate));
                        //EXPECT_CALL(*this, deallocate(_, 1)).Times(AtMost(4));
                  }
